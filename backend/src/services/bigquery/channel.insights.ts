...
export async function getChannelInsights(videoIds: string[]) {
  ...
  const [rows] = await getBigQueryClient().query({ query: `
    WITH
      ...
      comments AS (
        SELECT video_id, COUNT(*) conversations
        FROM ${table(TABLE_NAMES.COMMENTS)}
        WHERE video_id IN (SELECT video_id FROM ids)
        GROUP BY video_id
      ),
      latest_workflows AS (
        SELECT w.*, ROW_NUMBER() OVER (PARTITION BY w.video_id, w.workflow_id ORDER BY w.updated_at DESC) AS row_number
        FROM ${table(TABLE_NAMES.RESPONSE_WORKFLOW)} w
        WHERE w.video_id IN (SELECT video_id FROM ids)
      ),
      needs AS (
        SELECT video_id, COUNT(*) needs_response
        FROM latest_workflows
        WHERE row_number = 1 AND resolution_status IN ('needs_response', 'unclear', 'snoozed')
        GROUP BY video_id
      ),
      latest_analysis AS (
        SELECT a.*, ROW_NUMBER() OVER (PARTITION BY a.video_id, a.comment_id ORDER BY a.analyzed_at DESC) AS row_number
        FROM ${table(TABLE_NAMES.COMMENT_ANALYSIS)} a
        WHERE a.video_id IN (SELECT video_id FROM ids) AND a.prompt_version = @prompt_version
      ),
      analyzed_videos AS (
