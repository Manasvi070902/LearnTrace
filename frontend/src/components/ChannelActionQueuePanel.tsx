...
export function ChannelActionQueuePanel({ queue }: { queue: ChannelActionQueue }) {
  ...
  const updateQueueItem = async (item: ChannelActionQueueItem, action: 'snoozed' | 'restored') => {
    setLoadingId(item.workflowId); setError(null);
    try {
  ...
  return <section className="channel-action-queue">
    ...
    {sourceItems.length === 0 ? <div className="channel-empty"><strong>No {showSnoozed ? 'snoozed' : 'pending'} follow-ups.</strong></div> : <>
      ...
      <div className="channel-action-items"><h3>{showSnoozed ? 'Snoozed follow-ups' : selectedVideoId ? 'Pending for selected video' : 'Highest-priority follow-ups'}</h3><div className="channel-queue-rows">{visibleItems.slice(0, 8).map((item, index) => <article className={`priority-item channel-queue-card ${item.priority === 'high' ? 'priority-evidence-attention priority-high' : item.priority === 'medium' ? 'priority-evidence-repeated' : 'priority-evidence-individual'}`} key={item.workflowId}><span className="priority-number">{String(index + 1).padStart(2, '0')}</span><span className="priority-copy"><span className="priority-category">{showSnoozed ? 'SNOOZED' : item.priority === 'high' ? 'NEEDS ATTENTION' : item.priority === 'medium' ? 'REPEATED' : 'FOLLOW-UP'}</span><strong>{queueTitle(item.title)}</strong><small>{item.supportingLearners} learner{item.supportingLearners === 1 ? '' : 's'} · {item.videoTitle}</small></span><span className="channel-queue-card-actions">{!showSnoozed && <button type="button" className="snooze" onClick={() => void updateQueueItem(item, 'snoozed')} disabled={loadingId === item.workflowId}>{loadingId === item.workflowId ? 'Saving…' : 'Snooze'}</button>}<button type="button" className="view-details" onClick={() => void openTask(item)} disabled={loadingId === item.workflowId}>{loadingId === item.workflowId ? 'Loading…' : 'View details →'}</button></span></article>)}</div></div>
    </>}
