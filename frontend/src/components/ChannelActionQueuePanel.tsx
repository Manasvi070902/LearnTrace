...
export function ChannelActionQueuePanel({ queue }: { queue: ChannelActionQueue }) {
  ...
  const [pendingAction, setPendingAction] = useState<{ workflowId: string; type: 'details' | 'snoozed' | 'restored' } | null>(null);
  ...
  const updateQueueItem = async (item: ChannelActionQueueItem, action: 'snoozed' | 'restored') => {
    setPendingAction({ workflowId: item.workflowId, type: action }); setError(null);
    try {
  ...
  const openTask = async (item: ChannelActionQueueItem) => {
    setPendingAction({ workflowId: item.workflowId, type: 'details' }); setError(null);
    try {
